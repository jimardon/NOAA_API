...
function init()
{
    ...
    base_url_origin = "https://tidesandcurrents.noaa.gov"
}
...
function setupStations() {
    ...
    //get a list of many stations calling different station types from the metadata API
      ...
      for (var i1 = 0, j1 = types.length; i1 < j1; i1++) {
        ...
        get_url = 'https://noaastationsapi.s3.amazonaws.com/' + types[i1] + '.json';
...
function getData(url,type){
    ...
    alert("Attempting to get data from:" + url);
...
function FillStation(sta_num)
{
    ...
    {
        if(sta_num != "" && (all_stations[i].id).includes(sta_num)) //for station IDs
        {
            matches.push(all_stations[i]);
            fill_id = all_stations[i].id;
            fill_name = all_stations[i].name;
        }
        else if(sta_num != "" && (all_stations[i].id.toUpperCase()).includes(sta_num)) //for currents
        {
            fill_id = all_stations[i].id;
            fill_name = all_stations[i].name;            
        }
    }
    autocompleteClick(fill_name, fill_id,'d_stationselect','d_stationselect_matches');
}
...
function autocompleteClick(
    ...
    if ( d_station_id != '') {
        ...
        var get_data_url = '/mdapi/latest/webapi/stations/' + d_station_id + '/datums.json?units=' + d_units;
...
function URLlist()
{
    b_yy = d_bdate.substring(0, 4);
    b_mm = d_bdate.substring(4, 6);
    b_dd = d_bdate.substring(6, 8);

    e_yy = d_edate.substring(0, 4);
    e_mm = d_edate.substring(4, 6);
    e_dd = d_edate.substring(6, 8);

    var url_list = [];
    var new_aux = "";
    var start_date = b_yy + "-" + b_mm + "-" + b_dd;
    var end_date = e_yy + "-" + e_mm + "-" + e_dd;    
    var diff_days =  Math.floor(( Date.parse(end_date) - Date.parse(start_date) ) / 86400000);

    var diff_1y = Math.ceil(diff_days/365);
    var diff_5d = Math.ceil(diff_days/5);
    var diff_31d = Math.ceil(diff_days/31);

    var d = new Date(b_yy + "-" + b_mm + "-" + b_dd);
    d.setDate(d.getDate() + 1);

    if(diff_1y > 1 && URLvariable == "one_year")
    {    
        for (var i = 0; i < diff_1y; i++) {
        
            var edit_aux = new_url;
            var begd = new Date(d);
            var endd = new Date(d);
        
            begd.setDate(begd.getDate() + 365*i);
            endd.setDate(endd.getDate() + 365*(i+1));

            new_aux = edit_aux.replace(d_bdate, begd.getFullYear() + "" + ("0" + (begd.getMonth()+1)).slice(-2) + "" + ("0" + begd.getDate()).slice(-2));
            if (i+1 == diff_1y){
            new_aux = new_aux.replace(d_edate, e_yy + e_mm + e_dd);
            }
            else{
            new_aux = new_aux.replace(d_edate, endd.getFullYear() + "" + ("0" + (endd.getMonth()+1)).slice(-2) + "" + ("0" + endd.getDate()).slice(-2));
            } 
            url_list.push(new_aux);
        }
        downloadAll(url_list)
    }
    else if(diff_5d > 5 && URLvariable == "five_days")
    {    
        for (var i = 0; i < diff_5d; i++) {
        
            var edit_aux = new_url;
            var begd = new Date(d);
            var endd = new Date(d);
        
            begd.setDate(begd.getDate() + 5*i);
            endd.setDate(endd.getDate() + 5*(i+1));
    ...
    else if(diff_31d > 5 && URLvariable == "31_days")
    {    
        for (var i = 0; i < diff_31d; i++) {
        
            var edit_aux = new_url;
            var begd = new Date(d);
            var endd = new Date(d);
        
            begd.setDate(begd.getDate() + 31*i);
            endd.setDate(endd.getDate() + 31*(i+1));
...
function downloadAll(urls){
    var zip = new JSZip();
    var count = 0;
    var zipFilename = "COOP_NOAA_" + station_name + "_" + d_product + ".zip";
    const promises = [];
    ...
    urls.forEach(function(url){
        ...
        JSZipUtils.getBinaryContent(url, function (err, data) {
            ...
            zip.file(station_name + "_" + d_product + "_" + count + ".csv", data, {binary:true});
            count++;
